...
def run_migration():
    ...
    with app.app_context():
        try:
            ...
            if result > 0:
                logger.info("A coluna email_verified já existe na tabela user.")
                return
            
            # Adicionar a coluna email_verified
            add_column_query = text("ALTER TABLE user ADD COLUMN email_verified BOOLEAN DEFAULT FALSE")
            ...
            db.session.commit()
            logger.info("Coluna email_verified adicionada com sucesso à tabela user.")

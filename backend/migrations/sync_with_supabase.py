...
def backup_data(db):
    ...
    try:
        ...
        logger.info(
            f"Backup de {len(data_backup['users'])} usuários realizado")
    ...
    try:
        ...
        logger.info(
            f"Backup de {len(data_backup['alerts'])} alertas realizado")
    except Exception as e:
...
def run_migration():
    ...
    with app.app_context():
        try:
            ...
            if user_id_type and (
                    'VARCHAR' in user_id_type.upper() or 'STRING' in user_id_type.upper()):
                logger.info(
                    "A coluna user.id já é do tipo String/VARCHAR. Migração não necessária.")
                return
            ...
            for user_data in data_backup['users']:
                ...
                if not user_data.get('email'):
                    logger.warning(
                        f"Usuário sem email encontrado, ignorando: {user_data}")
                    continue
            ...
            # Commit dos usuários antes de restaurar os alertas (devido às
            # chaves estrangeiras)
            db.session.commit()
            ...
            if data_backup.get('alerts'):
                ...
                for alert_data in data_backup['alerts']:
                    ...
                    if isinstance(
                            ...
                            int) or isinstance(
                            alert_data.get('resolved_by'),
                            int):
                        logger.warning(
                            "Alertas com referências a IDs de usuário numéricos não podem ser restaurados")
                        continue

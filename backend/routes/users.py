...
def generate_more_users(count=15):
    names = ["Pedro", "Julia", "Rafael", "Camila", "Lucas", "Amanda", "Fernando", "Larissa", 
             "Gabriel", "Bianca", "Roberto", "Carla", "Rodrigo", "Patrícia", "Marcos"]
    surnames = ["Lima", "Costa", "Pereira", "Almeida", "Souza", "Oliveira", "Rodrigues", 
                "Ferreira", "Santos", "Ribeiro", "Gomes", "Carvalho", "Martins", "Silva"]
    domains = ["gmail.com", "outlook.com", "hotmail.com", "empresa.com.br", "medico.com.br"]
    ...
    for i in range(count):
        ...
        if days_ago == 0:
            last_login = "Hoje, " + f"{random.randint(8, 21)}:{random.randint(0, 59):02d}"
        elif days_ago == 1:
            last_login = "Ontem, " + f"{random.randint(8, 21)}:{random.randint(0, 59):02d}"
        else:
...
def reset_password(user_id):
    ...
    # atualizaria no banco de dados e enviaria por email
    
    # Simular geração de senha aleatória
    new_password = ''.join(secrets.choice(string.ascii_letters + string.digits) for _ in range(10))
    
    # Aqui você atualizaria o hash da senha no banco
...
def update_status(user_id):
    ...
    if not new_status or new_status not in ['active', 'inactive']:
        return jsonify({"error": "Status inválido. Deve ser 'active' ou 'inactive'."}), 400
    
    # Atualizar o status do usuário
    user['status'] = new_status
    
    return jsonify({
        "message": f"Status do usuário atualizado para '{new_status}' com sucesso.",
        "user": user
    })
...
def create_user():
    ...
    if role not in ['admin', 'manager', 'user']:
        return jsonify({"error": "Função inválida. Deve ser 'admin', 'manager' ou 'user'."}), 400
...
def update_user(user_id):
    ...
    if email:
        ...
        if email != user['email'] and any(u['email'] == email and u['id'] != user_id for u in dummy_users):

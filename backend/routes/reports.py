...
from models import db, User, Alert, AuthLog, JSONType
...
def export_report():
    ...
    if format_type not in ['csv', 'json', 'pdf']:
        return jsonify({'msg': 'Formato não suportado. Use csv, json ou pdf'}), 400
    ...
    try:
        ...
        if report_id == 'all_alerts':
            ...
            headers = ['ID', 'Tipo', 'Severidade', 'Timestamp', 'Resolvido', 'Resolvido Por', 'Usuário ID']
            data_rows = []
            ...
            if format_type == 'csv':
                ...
                return jsonify({'msg': 'Relatório gerado com sucesso', 'data': csv_data, 'format': 'csv'})
            
            elif format_type == 'json':
                json_data = generate_json(data_rows, ['id', 'type', 'severity', 'timestamp', 'resolved', 'resolved_by', 'user_id'])
                return jsonify({'msg': 'Relatório gerado com sucesso', 'data': json_data, 'format': 'json'})
            
            elif format_type == 'pdf':
                # Em uma implementação real, geraria o PDF
                return jsonify({'msg': 'Exportação de PDF disponível apenas para download direto'})
        
        # Relatório: Lista de usuários
        elif report_id == 'all_users':
            users = User.query.all()
            
            headers = ['ID', 'Nome', 'Email', 'Função', 'Status', 'Criado em', 'Último login']
            data_rows = []
            ...
            if format_type == 'csv':
                ...
                json_data = generate_json(data_rows, ['id', 'name', 'email', 'role', 'status', 'created_at', 'last_login'])
                return jsonify({'msg': 'Relatório gerado com sucesso', 'data': json_data, 'format': 'json'})
        ...
        # Relatório: Dados do dashboard
            ...
            if format_type == 'json':
                ...
                writer.writerow(['Relatório de Dashboard', f'Gerado em: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}'])
                writer.writerow([])
                ...
                for i in range(len(days)):
                    writer.writerow([days[i], acessos_semana[i], tentativas_bloqueadas[i]])
        ...
        return jsonify({'msg': f'Erro ao gerar relatório: {str(e)}', 'status': 'error'}), 500

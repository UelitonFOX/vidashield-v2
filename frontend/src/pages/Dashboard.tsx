...
export const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        // Fazer todas as requisições em paralelo para otimizar carregamento
        const [statsResponse, insightsResponse, alertasResponse] = await Promise.all([
        ...
        if (!statsResponse.ok || !insightsResponse.ok || !alertasResponse.ok) {
    ...
    const usarDadosMockados = () => {
      ...
      // Mockando usuários bloqueados
      const mockUsuariosBloqueados: UsuarioBloqueado[] = [
        { 
          id: 1, 
      ...
      setUsuariosBloqueados(mockUsuariosBloqueados);
      setLastUpdate(new Date());
    };
